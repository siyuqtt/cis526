...
class rule_hyp:
	def __init__(self, en=[], pe_given_f=1, pf_given_e=1, pe_given_f_lex = 1, pf_given_e_lex = 1, rarity=0, phrase_penalty = 0, align=[]):
		self.en=en
		...
		self.align=align
		w = [0.12817156400888038, 0.3544745566666286, 0.3391146050127749, 0.24257143336186784, 0.402106143732073, -0.1452664461926249]
		...
		for start in range(len(oris) - span +1):
			...
			if span==1: #span==1
				if not rules.has_key(tuple(oris[start:end])):
					...
					chart[(start,end)].append(chart_entry(en=oris[start:end], tmprob=20, lmprob=en_lmscore))
